...
function gsapInit() {
  const containers = document.querySelectorAll("[data-fls-anim-clouds]");
  containers.forEach((container) => {
    const clouds = container ? container.querySelectorAll("div") : [];
    if (clouds.length) {
      clouds.forEach((el) => el.style.willChange = "opacity, transform");
      gsapWithCSS.fromTo(
        clouds,
        { opacity: 0, x: 10 },
        {
          ...
          onComplete: () => clouds.forEach((el) => el.style.willChange = "")
        }
      );
    }
  });
